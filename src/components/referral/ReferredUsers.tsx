  ...
  return (
    <div className=" w-full max-w-3xl mx-auto p-4">
      ...
      <div className="flex justify-center mb-6">
        <p className="  rounded-lg p-4 break-words w-full max-w-md text-center">
          {invitationLink}
        </p>
      </div>
      ...
      <div className="flex justify-evenly items-center mb-8">
        <div className="text-center flex flex-col">
          <button className="bg-blue rounded p-3 flex items-center justify-center transition duration-300"
            onClick={copyToClipboard}
          >
            <FaLink className=" text-xl" />
          </button>
          <h2 className="mt-2 text-sm ">{isCopied ? 'Copied!' : 'Copy'}</h2>
        </div>

        <div className="text-center flex flex-col">
          <button
            className="bg-blue  py-2 px-4 rounded-lg flex items-center justify-center"
            onClick={() => {
              window.open(
          ...
          <h2 className="mt-2 text-sm ">Send</h2>
        </div>

        <div className="text-center flex flex-col">
        <button
          className="bg-blue rounded p-3 flex items-center justify-center transition duration-300"
          onClick={() => {
            const shareData = {
              title: "Earn Rewards!",
        ...
        >
          ...
          <h2 className="mt-2 text-sm ">Share</h2>
        </div>
      </div>
    ...
    referrals.map(({ balance, firstName, lastName, userImage }, idx) => (
      <div key={idx} className="flex items-center justify-between py-4 border-b border-gray-700">
        <div className="flex items-center">
          <div className="mr-4  rounded-full w-12 h-12 flex items-center justify-center">
            <p>{idx + 1}</p>
          </div>
          <div className="flex items-center">
            {userImage ? (
